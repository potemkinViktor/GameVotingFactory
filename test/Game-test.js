...
!developmentChains.includes(network.name)
  ...
  : describe("Game", async function () {
      ...
      beforeEach("Deploy the contract", async function () {
        ...
        await hardhatVrfCoordinatorV2Mock.fundSubscription(1, ether("7"));
        ...
        expect(await game.startGame()).to.equal(
        ...
        expect(
          await mock.connect(user1).approve(game.address, ether("0.5"))
        ).to.ok;
        expect(
          await mock.connect(user2).approve(game.address, ether("0.5"))
        ).to.ok;
        expect(
          await mock.connect(user3).approve(game.address, ether("0.5"))
        ).to.ok;

        expect(await mock.transfer(owner.address, ether("0.5"))).to.ok;
        expect(
          await mock.connect(owner).approve(gameFactory.address, ether("0.5"))
        ).to.ok;
      });
      ...
      describe("GameFactory", async function () {
        it("startGame", async function () {
          await expect(
            gameFactory
              .connect(owner)
              .startGame(mock.address, ether("0.5"), 3, 101)
          ).to.be.revertedWith("Your number should be less or equals to 100");
          await expect(
            gameFactory
              .connect(owner)
              .startGame(mock.address, ether("0"), 3, 57)
          ).to.be.revertedWith("Deposit shoul be more than ZERO");
          // await expect (gameFactory.connect(owner).startGame("0", ether("0.5"), 3, 57)).to.be.revertedWith("Wrong token address");
          expect(
            await gameFactory
              .connect(owner)
              .startGame(mock.address, ether("0.5"), 0, 57)
          );
          await expect(
            gameFactory
              .connect(owner)
              .startGame(mock.address, ether("0.5"), 0, 57)
          ).to.be.revertedWith("You can create just one game for one address");
      ...
      describe("Game", async function () {
        it("REQUIRE", async function () {
          await expect(game.connect(user1).participate(101)).to.be.revertedWith(
            "Your number should be less than 100"
          );

          await skipTimeTo((await getLatestBlockTimestamp()) + FIVE_MIN);
          await expect(game.connect(user2).participate(55)).to.be.revertedWith(
            "Game Over"
          );
        });
        ...
        it("participate and getting winner", async function () {
          ...
          await expect(game.connect(user1).participate(100)).to.be.revertedWith(
            "You are already in game"
          );
          // expect (game._msgSender()).to.equal(user1.address);
          // expect (await game._msgSender()).to.equal(user1.address);

          expect(await game.connect(user2).participate(59))
            .to.ok.to.emit(game, "NewGamer")
          ...
          await expect(game.connect(user3).participate(99)).to.be.revertedWith(
            "Limit of users"
          );
          ...
          await expect(game.getWinner()).to.be.revertedWith(
            "Game ended, tokens transfered to winners"
          );
        });
      });
    });
